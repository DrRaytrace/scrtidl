...
pro rtwlseg,sbtot,sbpol,snetot,imsize=imsize,$
             fovpix=fovpix,segment=segment,angle=angle,nbpix=nbpix,$
...
;+
;  $Id: rtwlseg.pro,v 1.3 2006-11-09 19:43:30 thernis Exp $
;
; PURPOSE:
;  Perform raytracing along a straight line segment profile
;
; CATEGORY:
...
; segment : [[x1,y1],[x2,y2]] defines the segment extremities, 
;           in image pix. 
; angle : if defined the segment goes from the image center and has
;         nbpix lenght. Angle is overread by segment.
...
if n_elements(segment) ne 0 then segment=float(segment)
if n_elements(angle) ne 0 then angle=float(angle)
if n_elements(nbpix) eq 0 then nbpix=255L else nbpix=long(nbpix)
...
rtinstrpresets,instr,imsize,fovpix,obsang,imszratio,secchiab=secchiab

; ---- define the default segment
if n_elements(segment) eq 0 and n_elements(angle) ne 0 then begin
    center=rtcalccntr(fovpix,obsang,imsize)
    point=center+float(nbpix)*[cos(angle),sin(angle)]
    segment=[[center],[point]]
endif else segment=[[0.,0.],[10,0]]
...
s=call_external(getenv('RT_LIBFILE'),$
                ...
                crpix,nbpix,segment,quiet,hlonlat,limbdark,$
                obslonlat,obslonlatflag,unload=unload)
...
mkhdr,h0,btot
sxaddpar,h0,'COMMENT','Generated with raytrace software - NRL'
...
sxaddpar,h0,'SEGMENTX1',segment[0,0],'pix'
sxaddpar,h0,'SEGMENTY1',segment[1,0],'pix'
sxaddpar,h0,'SEGMENTX2',segment[0,1],'pix'
sxaddpar,h0,'SEGMENTY2',segment[1,1],'pix'
